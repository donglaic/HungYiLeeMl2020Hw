...
#         self.theta = np.random.random((X.shape[1],1))
        ...
        step = np.zeros(self.theta.shape) + 1e-8
        ppos = 1
        for epoch in range(self.epoches):
            ...
            for idx in range(int(np.floor(X.shape[0]/self.batch_size))):
                ...
                step = step + grad**2
#                 print('step shape',step[:5])
#                 print('grad shape',grad[:5])
#                 print('nnp.sqrt(step)',np.sqrt(step)[:5])
#                 print('self.lr/(np.sqrt(step))*grad',(self.lr/(np.sqrt(step))*grad)[:5])
                self.theta = self.theta - self.lr/(np.sqrt(step))*grad # Adagrad
